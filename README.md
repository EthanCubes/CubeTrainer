...
2. For each of the 20 moves in the scramble, a random basic move is picked (U, L, F, R, B, or D). At this stage there are no prime moves or double moves yet.
...
2. A scramble is generated by getting a random algorithm from the algorithm set and doing the reverse of it. This involves doing both the reverse of the specified move, and also reversing the sequence in which the moves are executed. For example, R U L' becomes L U' R'.
...
- Note that I only used local storage because I was too lazy to learn IndexedDB. Local storage has limitations of only being able to store 5-10MB, which should be enough for this project.


## Credits
- AI was used to indentify bugs.
- Algorithm sets from sites like [JPerm.net](https://www.jperm.net) and [Ruwix](https://www.ruwix.com) were converted into a readable format by AI because it would've taken forever to do manually, and would also be very prone to human error.
